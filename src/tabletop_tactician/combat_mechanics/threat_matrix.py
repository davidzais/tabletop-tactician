        ...
        for defender, d_label in zip(merged_defender.units, defender_labels):
            for phase in (WeaponType.RANGED, WeaponType.MELEE):
                ...
                current_matchup = CombatMatchup(
                    ...
                    combat_phase=phase,
                    damage=damage,
                    wound_pool=defender.wounds,
                    defender_points=defender.points,
                )
                combat_matchups.append(current_matchup)
    return combat_matchups


def build_threat_overview(
    my_army: Army, enemy_army: Army, covered_enemy_labels: dict[str, str], enemy_name_lookup: dict[str, str]
):
...
def assign_targets(attacker: Army, defender: Army) -> tuple[dict[str, tuple], list]:
    ...
    for r, c in zip(row_ind, col_ind):
        holder[row_labels[r]] = (column_labels[c], best_phase_matchup[(row_labels[r], column_labels[c])][0], grid[r][c])
