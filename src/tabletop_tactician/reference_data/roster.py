...
def load_roster(text: str) -> Army:
    ...
    if not res["ok"]:
        raise ValueError(f"Failed to import roster: {res.get('roster', {}).get('diagnostics')}")
    ...
    units = [
        FieldedUnit(
            ...
            points=u["points"],
            composition=[UnitComposition(g["model_name"], g["count"]) for g in u.get("loadout_groups") or []],
