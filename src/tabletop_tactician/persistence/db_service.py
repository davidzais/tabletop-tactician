...
from tabletop_tactician.models.reports import ReportModel
...
def mark_report_done(job_id: str, user_id: str, result: str) -> None:
    with get_session_maker().begin() as session:
        try:
            ...
            stmt = update(UserTable).where(UserTable.user_id == user_id).values(num_reports_run=UserTable.num_reports_run + 1)
